...
//--------------------------------------------------------------------------------------------------------------
/*Módulo 1:Encontrar el tamaño de muestra correcto para conseguir una mayor certeza en la información,el usuario selecciona si serán 5 o 10 tiempos y los ingresa*/
...
/*Aproxima el valor del tamaño de muestra a un entero que se encuentre en la tabla de referencia*/
function aproximar()
{
    if (indicador<1)
    {
...
/*Busca el tamaño de muestra arrojado en el archivo json con los valores de referencia*/
function buscarTamanio()
{
    ...
    .then((tamaños)=>
    {
        ...
        {
            document.getElementById("tamMuestraText").innerHTML=`<u/>El tamaño de muestra o número de mediciones que deben realizarse para obtener el tiempo promedio con un 95% de confianza es de: ${sampleSize}`
            capturaTamMuestral();
        }
    })
}

/*Muestra una sección especifica según botón seleccionado por el usuario*/
...
/*Captura el tamaño de muestra ingresado por el usuario una vez lo escribe y lo guarda en el local Storage y genera los campos de acuerdo a dicho tamaño*/
function capturaTamMuestral(e)
{
    if (once==0){
        Toastify({
            text: "Tamaño de muestra ingresado exitosamente",
            duration: 3800,
        }).showToast();
    }
    once=once+1;
...
//-------------------------------------------------------------------------------------------------------------
/*Módulo 2:Selección del operario más calificado e ingreso de la información de referencia*/
...
/*Lee la información del operario de referencia*/
function leerInfRef()
{
    Toastify({
    ...
    tamaño de muestra:<br> ${sampleSize} datos, <br>
    nombre: ${nameOperRef}, <br>
    experiencia: ${experienceOperRef} años<br>`
...
/*Muestra las holguras variables que el usuario vaya seleccionando*/
...
/*Muestra la sección para añadir una holgura variable por política*/
...
/*Obtiene la información del objeto guardado en el local Storage*/
...
Carga de información  de los tiempos de diferentes operarios
*/

/*Captura el número de operarios ingresados por el usuario y crea los campos necesarios para ingresar la información de estos*/
function capturanumOperarios(e)
{
    ...
    for (let j=0;j<=(numOperarios-1);j++)
    {
        ...
        let p=document.createElement("p");
        p.innerHTML=`a continuación ingrese los tiempos del operario: ${j+1} usando "." para decimales`;
...
/*Lee la información de los operarios ingresada por el usuario*/
...
/*Calcula el tiempo promedio de los operarios según los datos ingresados*/
...
/*Calcula el tiempo estándar básico, es decir que suma las holguras básicas al tiempo promedio*/
function calcularEstMult()
{
    ...
    for (let i=0;i<=(numOperarios-1);i++)
    {
        ...
        p.innerHTML=document.getElementById(`estandP${i+1}`).innerHTML=`El tiempo estandar básico del operario ${operario[i].nombre} es:${estandarTimeRef.toFixed(2)} minutos`;
        p.classList.add("respuesta")
    }
}

/*El usuario puede seleccionar si usará las mismas holguras variables en todos los operarios a medir o si usará holguras diferentes para cada operario*/
...
/*Muestra la sección de selección de holguras variables*/
...
/*Genera una lista desplegable para seleccionar un operario específico y muestra las sección de holguras variables*/
function mostrarHolgurasVarInd()
{
        generarListaDesp("containerOperarios","operarios");
        mostrarHolgurasVarMult();
        detonante=1;
}

/*Crea una lista desplegable con los nombres de los operarios registrados por el usuario*/
function generarListaDesp(id,nombreId)
{
    ...
    label.classList.add("listDespLabel");
    document.getElementById(id).appendChild(label).appendChild(select);
}

/*Agrega las mismas holguras variables a los tiempos de todos los operarios*/
...
/*Describe las holguras seleccionadas*/
...
/*Limpia las holguras variables selccionadas*/
...
/*Muestra la sección de agregar holgura política*/
...
/*Cálcula qué porcentaje del tiempo final del operario de referencia es el tiempo del operario que sea seleccionado en la lista desplegable*/
function compararTiempos()
{
    ...
    for (j in operario)
    {
        let nombre=document.getElementById("operarios2").value;
        if (operario[j].nombre==nombre)
        {
            index=j;
...
/*Lleva el tiempo estándar final de los operarios a horas para determinar indicadores durante la jornada ingresada por el usuario*/
function converMin()
{
    ...
    for (j in operario)
    {
        let nombreBot=document.getElementById("operarios2").value;
...
/*Realiza cálculos empleando la librería Luxon para determinar con el tiempo estandar final de cada operario cuantas veces podría realizar la tarea estudiada entre dos fechas específicas*/
